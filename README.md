...
New in 2.2: the energy flow of the dashboard now reads **two ways, switchable and remembered**. The **schematic** is unchanged — it shows the topology of your installation. The new **Sankey** makes ribbon width the wattage, so the split reads at a glance: how much of your production goes straight to the house versus into the battery, and how much of your consumption never touches the SolarFlow at all — a proportion constant-width links simply cannot show. Two things it refuses to fake: a Sankey visually asserts that everything in comes back out, so the hub's residual is drawn explicitly as a grey “Pertes & conversion” (or “Écart de mesure”) ribbon rather than silently absorbed; and without a Smart CT the unmeasured grid→home draw keeps a **fixed, hatched, non-proportional** band — giving it a width would invent a number, dropping it would claim it is zero.

New in 2.1: **SunRoad reads your energy day at a glance** — home consumption as a continuous Helios-style blue curve running the full 24-hour circle of the scene (position is the hour, height is the watts, quarter-hour pins), a live **“⌂ n W” badge** above the house, and an **expected clear-sky production curve** (yellow) on the same ground circle, baseline and scale as the actual-production bars — the gap between expected and actual reads instantly, follows the ±48 h timeline and reacts to the orientation sliders. Plus two fixes: the Smart Meter 3CT no longer shows an empty History card (all-zero cloud payloads now count as “no history”), and local polling refuses a Smart CT answering at the SolarFlow address (explicit error instead of silent zeros after a DHCP change).

New in 2.0: **SunRoad** — your house, your neighborhood and the sun in a real 3D scene (native SceneKit, inspired by the Helios card for Home Assistant): OpenStreetMap building footprints and roads with your house auto-detected and highlighted (or **picked with a click** for an exact center), the sun as an actual directional light casting **real shadows** that sweep across the day, your panel arrays at their true azimuth/tilt, animated energy-flow beads paced by the live watts, a production ribbon along the sun's arc, real cloud cover veiling the light, a **±48 h timeline**, per-layer visibility checkboxes and a wall mode. SunRoad **replaces the Sun window**: its sidebar carries over the per-array orientation sliders (re-aiming the panels live in 3D), the ephemerides, twilights, theoretical output and weather cards, plus the 14-day production histogram.
...
- [x] v2.1 — Helios-style consumption curve (continuous 24 h circle, quarter-hour pins, live “⌂ W” badge above the house) and expected clear-sky production curve on the same scale as the actual bars; History no longer shows the Smart Meter 3CT with all-zero data; local poll rejects a Smart CT answering at the SolarFlow address
- [x] v2.2 — Sankey reading of the energy flow alongside the schematic (ribbon width = watts, explicit hub residual, non-proportional hatched band for the unmeasured grid→home draw), switchable and remembered
- [ ] v2.3 — SunRoad polish (street names, vegetation/water, neighborhood radius setting, mirrored cardinal letters fix), cloud `outputPower` mapping (~3 s home flow), zenSDK fault/error fields in the dashboard, Chinese localization, reorderable cards, widget refresh button (AppIntents), grid-draw daily history from the Smart CT
- [ ] v2.4 — off-peak/peak-hours optimizer (local scheduler via `POST /properties/write`)
